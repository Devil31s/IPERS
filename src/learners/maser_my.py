...
class maserQDivideLearner2:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        print("terminated is :", terminated.shape)
        print("****************************")
        mask = batch["filled"][:, :-1].float()
        mask[:, 1:] = mask[:, 1:] * (1 - terminated[:, :-1])
        avail_actions = batch["avail_actions"]
        indi_terminated = batch["indi_terminated"][:, :-1].float()
        print("indi_terminated is :", indi_terminated.shape)
...
##############################################################################################################################
        if self.goal =="maser":
            ...
            print("the shape of intrinsic_rewards is :", intrinsic_rewards.shape)
        ...
        if self.goal == "maser":
            ...
            print("我想知道的shape:",(self.lam*intrinsic_rewards_ind).shape)
            ...
            # q_selected_weight, selected_ratio = self.select_trajectory(masked_td_individual_error.abs(), ind_mask, t_env)
            ...
            print("th.mean(ind_mask, dim=-1).sum() is :",th.mean(ind_mask, dim=-1).sum())
            print("th.sum(masked_td_individual_error ** 2).sum(): ",th.sum(masked_td_individual_error ** 2).sum())
        ...
        q_rewards = self.cal_indi_reward(grad_qtot_qi, td_error, chosen_action_qvals, target_max_qvals, indi_terminated)
        print("the shape of q_rewards is :", q_rewards.shape)
        print("我想看看q_reward的值：", q_rewards)
        print("我想看看chosen_action_qvals_clone的值：", chosen_action_qvals_clone)
        ...
        print("the number of q_mask.sum() is ", q_mask.sum())
        print("the number of 0.01*(masked_q_td_error ** 2 * q_selected_weight).sum() is :", 0.01*(masked_q_td_error ** 2 * q_selected_weight).sum())
