...
##############################################################################################################################
        ...
        if self.goal == "maser":
            ...
            #if ( (q_selected_weight * ind_mask).sum().item() / (ind_mask.sum().item()) ) > 0.15:
            #    print("我自己的设计！！！！！！！！")
            #    individual_loss = th.sum(masked_td_individual_error ** 2 * q_selected_weight).sum() / th.mean(ind_mask, dim=-1).sum()
            #if ( (q_selected_weight * ind_mask).sum().item() / (ind_mask.sum().item()) ) < 0.15:
            #    print("小于50% 依旧使用他原来的loss!!!!")
            individual_loss = th.sum(masked_td_individual_error ** 2 * q_selected_weight).sum() / th.mean(ind_mask, dim=-1).sum()
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            ind_mask_elems = ind_mask.sum().item()
            self.logger.log_stat("q_selected_weight_mean",(q_selected_weight * ind_mask).sum().item() / (ind_mask_elems), t_env)
