...
#####################################################################################################
        ...
        for i in range(self.n_agents):
            intrinsic_reward_list.append(-th.norm(reward_ddqn_up[:, :, i, :], dim=2).reshape(batch.batch_size, observation.shape[1]))
        intrinsic_rewards_ind = th.stack(intrinsic_reward_list, dim=-1)      # (B,T,n_agents)
        #print("the shape of intrinsic_rewards_ind is:",intrinsic_rewards_ind.shape)
...
####################################################################
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("q_selected_weight_mean", (q_selected_weight * q_mask).sum().item() / (q_mask_elems),
                                 t_env)
